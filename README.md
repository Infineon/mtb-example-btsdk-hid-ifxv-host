# IFX-Voice LE Host Sample Application
...
4. Optionally, the PUART can be opened for console display. (115200, 8, N, 1, No Flow control)
   ...
   Follow the same steps for ifxv-remote application. (See README.md in ifxv-remote)
...
- Yellow/Green LED:
    * 	Slow blink (1 Hz), scanning for ifxv-remote devices.
    * 	Fast blink (4 Hz), ifxv-remote service discovery.
...
After service discovery completes, both host and remote's LED turns solid on to indicate the link is ready for audio testing. In Client Control, it should display the device capabilities and the audio configurations.

![Image](./images/linkup.png)

## Audio testing

1. Initiate live audio streaming from ifxv-host.
